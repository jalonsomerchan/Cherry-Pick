...
Este repositorio usa una base HTML, CSS y JavaScript estática que debe mantenerse ligera, traducible, modular, mantenible y preparada para desplegarse correctamente, incluyendo GitHub Pages cuando aplique.
...
- Mantener el soporte i18n estático del proyecto.
...
4. No dejar idiomas incompletos: toda clave nueva debe existir en todos los idiomas de `src/i18n/translations.js`.
5. No añadir dependencias si la solución puede hacerse razonablemente con HTML, CSS, JavaScript o APIs nativas.
6. No romper rutas relativas, despliegues en raíz/subruta, i18n, SEO básico, accesibilidad ni tests existentes.
...
- configuración y traducciones en `src/i18n/translations.js`.
...
- Si se toca i18n, comprobar que todos los idiomas siguen alineados.
...
- Extraer constantes compartidas a `src/data/` o un módulo equivalente.
...
- La fuente de verdad de idiomas y textos está en `src/i18n/translations.js`.
- Las traducciones viven en el objeto `translations`.
- No hardcodear textos visibles en componentes reutilizables.
- Usar `t(locale, key)` o atributos `data-i18n` / `data-i18n-attr` para textos de UI.
- Mantener todas las claves alineadas entre idiomas.
- Si se añade un idioma, debe añadirse su bloque completo en `translations`.
- Las rutas internas deben mantenerse relativas y localizadas cuando aplique.
...
1. Añadirla al idioma por defecto en `translations`.
2. Añadirla a todos los demás idiomas configurados.
3. Usarla mediante helpers o atributos de i18n.
...
- HTML estático.
- CSS nativo con variables globales.
- JavaScript nativo modular.
- Traducciones en `src/i18n/translations.js`.
- Assets estáticos en `assets/`.
- Tests básicos con `node:test`.
- Build estático con scripts de Node en `tools/`.

## Estructura importante

- `index.html`: home del idioma por defecto.
- `en/index.html`: home del idioma inglés.
- `src/i18n/translations.js`: textos traducibles.
- `src/data/game.js`: datos de dificultad y sprites.
- `src/scripts/cherryGame.js`: lógica del juego.
- `src/styles/global.css`: tokens, layout y estilos del juego.
- `assets/`: assets estáticos.
- `manifest.webmanifest`: manifest estático.
- `robots.txt`: robots estático.
- `tools/build.mjs`: build estático a `dist/`.
...
No crear enlaces internos o assets con rutas absolutas duras tipo `/archivo.svg`, `/assets/...` o `/ruta/` si deben funcionar dentro de una subcarpeta.

Usar rutas relativas cuando aplique:

- `assets/...` desde la home raíz.
- `../assets/...` desde páginas en subcarpetas como `en/`.
- `./` o `../` para navegación estática según la profundidad de la página.
...
- dominio raíz: servido desde `/`.
- subruta: servido desde `/nombre-del-repositorio/`.

### No romper i18n

No meter textos visibles directamente en componentes o páginas si son parte de la UI reutilizable. Añadir claves en `src/i18n/translations.js`.

Cuando se añada una clave nueva:

1. Añadirla en el idioma por defecto dentro de `src/i18n/translations.js`.
2. Añadirla en todos los demás idiomas configurados.
3. Usarla con `t(locale, key)` o `data-i18n`.
4. Mantener las claves alineadas entre idiomas.

### No añadir dependencias sin necesidad

Evitar librerías nuevas salvo que la tarea lo requiera claramente. Preferir HTML, CSS, JavaScript y APIs del navegador.
...
Los tests de i18n deben leer los idiomas reales desde `src/i18n/translations.js`.
...
- ¿Las rutas internas funcionan tanto en dominio raíz como en subruta?
- ¿Los assets, enlaces, canonical, Open Graph, manifest, robots y sitemap respetan `base` cuando aplica?
- ¿Los textos nuevos están en todos los idiomas de `translations`?
...
- Saltarse `src/i18n/translations.js`.
